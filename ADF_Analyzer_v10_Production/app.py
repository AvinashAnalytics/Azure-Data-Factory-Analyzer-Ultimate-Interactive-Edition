...
def load_custom_css():
    ...
    <style>
        ...
        * {
        ...
        .premium-header {
            background: linear-gradient(135deg, rgba(60, 70, 120, 0.95) 0%, rgba(80, 50, 120, 0.95) 100%);
        ...
        /* ════════════════════════════════════════════════════════════ */
        /* METRIC TILES - Optimized & Consistent */
        /* ════════════════════════════════════════════════════════════ */
        
        .metric-card, .metric-card-premium {
        ...
        .gradient-purple {
            background: linear-gradient(135deg, #6B5BFF 0%, #C67CFF 45%, #FF7CC6 100%);
            color: white;
            box-shadow: 0 10px 30px rgba(107, 91, 255, 0.15);
        }
        
        .gradient-pink {
            background: linear-gradient(135deg, #FFD1E6 0%, #FF7CC6 45%, #FFB86B 100%);
            color: white;
            box-shadow: 0 10px 30px rgba(255, 124, 198, 0.15);
        }
        
        .gradient-blue {
            background: linear-gradient(135deg, #7DD3FC 0%, #70ADF6 45%, #9B8CFF 100%);
            color: white;
            box-shadow: 0 10px 30px rgba(77, 144, 255, 0.12);
        }
        ...
        .gradient-orange {
            background: linear-gradient(135deg, #FFD6A5 0%, #FF9A76 45%, #FF6A88 100%);
            color: white;
            box-shadow: 0 10px 30px rgba(255, 106, 136, 0.12);
        }
        
        .gradient-teal {
            background: linear-gradient(135deg, #B2FEF7 0%, #7EE7C6 45%, #4FD1C5 100%);
            color: #032829;
            box-shadow: 0 10px 30px rgba(79, 209, 197, 0.1);
        }
        
        .gradient-fire {
            background: linear-gradient(135deg, #FFDDD6 0%, #FFB88C 45%, #FF7AA2 100%);
            color: white;
            box-shadow: 0 10px 30px rgba(255, 122, 162, 0.12);
        }
        
        .gradient-ocean {
            background: linear-gradient(135deg, #CDEBFF 0%, #A8E6CF 50%, #FDE2FF 100%);
            color: #06202a;
            box-shadow: 0 12px 28px rgba(12, 120, 160, 0.08);
        }
...
def safe_get_dataframe(sheet_name: str, *alternative_names: str) -> pd.DataFrame:
    ...
    for key, df in excel_data.items():
        try:
            if normalize(key) == target_norm and isinstance(df, pd.DataFrame):
                return df.copy()
        except Exception:
            continue
    ...
    for alt_name in alternative_names:
        ...
        for key, df in excel_data.items():
            try:
                if normalize(key) == alt_norm and isinstance(df, pd.DataFrame):
                    return df.copy()
            except Exception:
                continue
...
class ADF_Dashboard:
    ...
    def _render_sidebar_file_upload(self):
        ...
        if mode != 'analyze':
            ...
            <div class="sidebar-section">
                <h3 style="margin: 0 0 15px 0; color: #667eea; font-weight: 700; font-size: 1.1em;">
                    📁 Data Input
                </h3>
            </div>
            """, unsafe_allow_html=True)
            ...
            st.markdown("""
            <div class="sidebar-section">
                <h3 style="margin: 0 0 10px 0; color: #667eea; font-weight: 700; font-size: 1.1em;">
                    📁 Upload & Analyze Mode
                </h3>
    ...
    def _process_loaded_sheets(self, data: Dict[str, pd.DataFrame]):
        ...
        # Create normalized aliases for common variants
        def normalize(key: str) -> str:
            return re.sub(r"[_\s]+", "", str(key)).lower()
        
        aliases = {
            "linkedserviceusage": ["LinkedServiceUsage", "LinkedService_Usage"],
            "integrationruntimeusage": ["IntegrationRuntimeUsage", "IntegrationRuntime_Usage"],
            "globalparameterusage": ["GlobalParameterUsage", "Global_Parameter_Usage"],
            "datasetusage": ["DatasetUsage", "Dataset_Usage"],
        }
        
        for canonical, variants in aliases.items():
            if canonical in [normalize(k) for k in data.keys()]:
                continue
    ...
    def _generate_sample_data(self) -> Dict[str, pd.DataFrame]:
        ...
        return {
            "Summary": pd.DataFrame([
                ...
                {"Metric": "Analyzer Version", "Value": "10.1 Enhanced"},
            ...
            "ActivityCount": pd.DataFrame([
                ...
                {"ActivityType": "ExecuteDataFlow", "Count": 28, "Percentage": "22.2%"},
    ...
    def render_main_content_with_tabs(self):
        """Render main content with dual-mode tabs."""
        # Back button
        if st.button("◀️ Back to Launcher", key="main_back_launcher"):
            for k in ['app_mode', 'app_mode_selected']:
                if k in st.session_state:
                    del st.session_state[k]
            st.rerun()
    ...
    def render_generate_excel_tab(self):
        ...
        if st.button('▶️ Run Generator', type="primary", use_container_width=True):
            if not confirm_run:
        ...
        try:
            ...
            if produced_file.exists():
                ...
                if load_after:
                    ...
                    st.success('✅ Loaded into dashboard!')
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    @st.cache_data(ttl=300)
    def _get_cached_metrics(_self) -> Dict[str, int]:
        """
    ...
    @st.cache_data(ttl=600)
    def _render_lineage_charts(_self, lineage_df: pd.DataFrame):
    ...
    @st.cache_data(ttl=600)
    def _render_pipeline_distribution_chart(_self):
    ...
    def render_impact_analysis_tab(self):
        ...
        # Filters
        self._render_impact_filters()
        
        # Pipeline details (implementation continues in next part)
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    @st.cache_data(ttl=600)
    def _render_activity_bar_chart(_self, activity_df: pd.DataFrame):
    ...
    @st.cache_data(ttl=600)
    def _render_activity_pie_chart(_self, activity_df: pd.DataFrame):
    ...
    def render_dataflow_tab(self):
        ...
        with col2:
            total_sources = dataflow_df.get('Sources', pd.Series([0])).sum()
            st.metric("Total Sources", int(total_sources))
        with col3:
            total_sinks = dataflow_df.get('Sinks', pd.Series([0])).sum()
    ...
    def render_lineage_tab(self):
        ...
        if search:
            mask = filtered.apply(lambda x: search.lower() in str(x).lower(), axis=1).any(axis=1)
    ...
    def render_export_tab(self):
        ...
        with col1:
            if st.button("✅ Select All", use_container_width=True):
                st.session_state['export_sheets'] = sheet_names
        with col2:
            if st.button("❌ Clear All", use_container_width=True):
                st.session_state['export_sheets'] = []
        
        selected = st.multiselect(
            "Sheets to Export",
            sheet_names,
            default=st.session_state.get('export_sheets', sheet_names[:5])
        )
        
        st.session_state['export_sheets'] = selected
        ...
        with col2:
            ...
            if HAS_OPENPYXL:
                if st.button("📥 Download XLSX", type="primary", use_container_width=True):
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # DOCUMENTATION - Single Implementation (NO DUPLICATES)
    # ═══════════════════════════════════════════════════════════════════════
    ...
    def render_comprehensive_documentation(self):
        ...
        ✅ FIXED: This is the ONLY implementation (removed duplicate from original).
        """
    ...
    def _render_tile_reference(self):
        ...
        try:
            ...
            if tiles_path.exists():
                ...
                1. **Pipelines** - Total pipeline resources
                2. **DataFlows** - DataFlow resources  
            ...
            st.error(f"❌ Error loading tiles reference: {e}")
    ...
    def _render_python_files_reference(self):
        ...
        - **`adf_dashboard.py`** - Main Streamlit dashboard (THIS FILE)
    ...
    def _render_project_guide(self):
        ...
        - **Dual-Mode** - Generate Excel + Upload & Analyze
        - **Performance** - Smart caching for instant re-render
        - **Modern UI** - Glassmorphism design with animations
        - **Modular Code** - Clean architecture, easy to maintain
        """)
    ...
    def _render_configuration_guide(self):
        ...
        **Advanced Features:**
        - `enhanced_summary` - Executive dashboard
        - `advanced_dashboard` - Health score, complexity maps
        - `health_score` - Factory health indicator
        ...
        **Via File Editing:**
        1. Edit `enhancement_config.json`
    ...
    def render_upload_interface(self):
        ...
        with col1:
            if uploaded_file and st.button("📤 Load Excel", type="primary", use_container_width=True):
                self.load_excel_file(uploaded_file)
                st.rerun()
