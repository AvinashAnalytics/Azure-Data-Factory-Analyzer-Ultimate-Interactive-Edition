...
try:
    ...
    print(f"  Warning during pattern analysis: {e}")
...
try:
    ...
    if not df_datasets.empty and "Name" in df_datasets.columns:
        ...
        if unique_datasets_from_datasets_sheet >= max(total_source_datasets, total_target_datasets):
            ...
            print(f"    Some datasets may not be used in lineage")

except Exception as e:
    print(f"  Warning during cross-check: {e}")
...
1⃣  ANALYZER: Successfully processed real-world template
   - Template size: {os.path.getsize(template_file):,} bytes
   - Excel output generated: {excel_file}

2⃣  METRICS CALCULATION: All metrics computed correctly
