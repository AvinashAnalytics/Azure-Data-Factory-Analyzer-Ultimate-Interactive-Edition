...
def load_custom_css(theme: str | None = None):
    ...
    if theme == "light":
        ...
        :root {
            ...
            --header-text: #0f172a;
            /* Tile color variables for easy tuning */
            --tile-value-color: #ffffff;
            --tile-value-shadow: 0 2px 6px rgba(0,0,0,0.12);
            --tile-label-color: var(--heading-color);
            --tile-subtitle-color: var(--muted);
        ...
        :root {
            ...
            /* slightly stronger tile surface so pastel gradients read on dark */
            --tile-bg: rgba(255,255,255,0.08);
            --tile-border-opacity: 0.12;
            ...
            --header-text: #f8fafc;
            /* Tile color variables for easy tuning */
            --tile-value-color: var(--contrast-text);
            --tile-value-shadow: 0 2px 6px rgba(0,0,0,0.35);
            --tile-label-color: var(--heading-color);
            --tile-subtitle-color: var(--muted);
            --tile-icon-filter: none;
            --title-stroke: 0px transparent;
    ...
    .app-heading { color: var(--heading-color) !important; font-weight:600; letter-spacing:0.5px; }
        ...
        /* Dark theme tile overrides for better visibility */
        body[data-theme="dark"] .tile-purple,
        body[data-theme="dark"] .tile-pink,
        body[data-theme="dark"] .tile-blue,
        body[data-theme="dark"] .tile-green,
        body[data-theme="dark"] .tile-orange,
        body[data-theme="dark"] .tile-teal,
        body[data-theme="dark"] .tile-fire,
        body[data-theme="dark"] .tile-success,
        body[data-theme="dark"] .tile-warning,
        body[data-theme="dark"] .tile-danger {
            opacity: 1 !important;
        }
        ...
        body[data-theme="dark"] .tile-purple {
            ...
            box-shadow: 0 8px 28px rgba(102,126,234,0.15), inset 0 1px 0 rgba(255,255,255,0.08);
        }

        body[data-theme="dark"] .tile-pink {
            background: linear-gradient(135deg, rgba(240,147,251,0.15), rgba(245,87,108,0.12));
            border: 1px solid rgba(240,147,251,0.25);
            box-shadow: 0 8px 28px rgba(240,147,251,0.15), inset 0 1px 0 rgba(255,255,255,0.08);
        }

        body[data-theme="dark"] .tile-blue {
            background: linear-gradient(135deg, rgba(79,172,254,0.15), rgba(0,242,254,0.12));
            border: 1px solid rgba(79,172,254,0.25);
            box-shadow: 0 8px 28px rgba(79,172,254,0.15), inset 0 1px 0 rgba(255,255,255,0.08);
        }

        body[data-theme="dark"] .tile-green {
            background: linear-gradient(135deg, rgba(67,233,123,0.15), rgba(56,249,215,0.12));
            border: 1px solid rgba(67,233,123,0.25);
            box-shadow: 0 8px 28px rgba(67,233,123,0.15), inset 0 1px 0 rgba(255,255,255,0.08);
        }

        body[data-theme="dark"] .tile-orange {
            background: linear-gradient(135deg, rgba(250,112,154,0.15), rgba(254,225,64,0.12));
            border: 1px solid rgba(250,112,154,0.25);
            box-shadow: 0 8px 28px rgba(250,112,154,0.15), inset 0 1px 0 rgba(255,255,255,0.08);
        }

        body[data-theme="dark"] .tile-teal {
            background: linear-gradient(135deg, rgba(48,207,208,0.15), rgba(51,8,103,0.12));
            border: 1px solid rgba(48,207,208,0.25);
            box-shadow: 0 8px 28px rgba(48,207,208,0.15), inset 0 1px 0 rgba(255,255,255,0.08);
        }

        body[data-theme="dark"] .tile-fire {
            background: linear-gradient(135deg, rgba(245,87,108,0.15), rgba(254,204,27,0.12));
            border: 1px solid rgba(245,87,108,0.25);
            box-shadow: 0 8px 28px rgba(245,87,108,0.15), inset 0 1px 0 rgba(255,255,255,0.08);
        }

        body[data-theme="dark"] .tile-success {
            background: linear-gradient(135deg, rgba(34,197,94,0.15), rgba(21,128,61,0.12));
            border: 1px solid rgba(34,197,94,0.25);
            box-shadow: 0 8px 28px rgba(34,197,94,0.15), inset 0 1px 0 rgba(255,255,255,0.08);
        }
        ...
        body[data-theme="dark"] .tile-danger {
            ...
            box-shadow: 0 8px 28px rgba(239,68,68,0.15), inset 0 1px 0 rgba(255,255,255,0.08);
        }

        /* Theme-specific explicit overrides removed - tiles now follow
           the global CSS variables (var(--contrast-text), --heading-color,
           --muted, etc.). This preserves the original design while allowing
           central theme tuning via root variables. */
        ...
        /* ═══════════════════════════════════════════════════════════════ */
        /* ULTRA-MODERN GRADIENT VARIANTS (pastel multi-stop, screenshot-inspired) */
        /* ═══════════════════════════════════════════════════════════════ */

        .gradient-purple {
            background: linear-gradient(135deg, #ecebff 0%, #e8ddff 55%, #ffe3f3 100%);
            color: var(--contrast-text);
            box-shadow: 0 6px 18px rgba(107, 91, 255, 0.08);
        }

        .gradient-pink {
            background: linear-gradient(135deg, #ffe6f3 0%, #ffd9ef 45%, #ffe8d2 100%);
            color: var(--contrast-text);
            box-shadow: 0 6px 18px rgba(255, 124, 198, 0.08);
        }

        .gradient-blue {
            background: linear-gradient(135deg, #e0f6ff 0%, #dde9ff 55%, #e8e4ff 100%);
            color: var(--contrast-text);
            box-shadow: 0 6px 18px rgba(77, 144, 255, 0.08);
        }

        .gradient-green {
            background: linear-gradient(135deg, #e6fff3 0%, #d9faee 55%, #daf5e9 100%);
            color: var(--contrast-text);
            box-shadow: 0 6px 18px rgba(52, 211, 153, 0.06);
        }

        .gradient-orange {
            background: linear-gradient(135deg, #fff0dd 0%, #ffe3d2 50%, #ffe0e7 100%);
            color: var(--contrast-text);
            box-shadow: 0 6px 18px rgba(255, 106, 136, 0.07);
        }

        .gradient-teal {
            background: linear-gradient(135deg, #e2fffc 0%, #dcfaf2 55%, #daf4f3 100%);
            color: var(--contrast-text);
            box-shadow: 0 6px 18px rgba(79, 209, 197, 0.06);
        }

        .gradient-fire {
            background: linear-gradient(135deg, #ffe8e3 0%, #ffd9c9 55%, #ffe3ed 100%);
            color: #3d1111;
            box-shadow: 0 6px 18px rgba(255, 122, 162, 0.07);
        }

        .gradient-ocean {
            background: linear-gradient(135deg, #eaf7ff 0%, #e6fff5 50%, #fdefff 100%);
            color: var(--contrast-text);
            box-shadow:
                0 6px 18px rgba(12, 120, 160, 0.05),
    ...
    # Add JavaScript to set data-theme attribute
    script = f"""
    <script>
        // Set data-theme attribute on body for CSS theme targeting
        document.body.setAttribute('data-theme', '{theme}');
    </script>
    """
    
    # Small runtime CSS that binds tile elements to theme variables so we can
    # tune colors centrally without editing many selectors.
    extra_css = """
    <style>
    /* Use the contrast text color knob so tiles follow overall theme contrast */
    .tile-value { color: var(--contrast-text) !important; text-shadow: var(--tile-value-shadow) !important; }
    .tile-label { color: var(--heading-color) !important; }
    .tile-subtitle { color: var(--muted) !important; }
    .tile-icon { color: var(--tile-accent-start) !important; filter: var(--tile-icon-filter) !important; }
    </style>
    """
    css += extra_css

    # Inject CSS using st.markdown to ensure it applies to the main page
    st.markdown(css, unsafe_allow_html=True)

    # Inject JS to set the theme attribute on the parent document body
    # We use st.components.v1.html for the script because st.markdown often strips <script> tags.
    # We try to access window.parent.document to affect the main app.
    js = f"""
    <script>
        try {{
            window.parent.document.body.setAttribute('data-theme', '{theme}');
        }} catch (e) {{
            console.log("Could not set theme on parent body:", e);
        }}
    </script>
    """
    try:
...
class ADF_Dashboard:
    ...
    def render_main_content_with_tabs(self):
        ...
        main_tabs = st.tabs(["⚙ Generate Excel", " Upload & Analyze", "📚 Documentation"])
    ...
    def render_enhanced_metrics(self):
        ...
        try:
            linked_services = get_count_with_fallback("LinkedServices", ["LinkedServices", "LinkedServiceUsage"])
        except Exception as e:
            # st.warning(f"Error calculating linked services count: {e}")
            linked_services = 0

        try:
            integration_runtimes = get_count_with_fallback("Integration Runtimes", ["IntegrationRuntimes", "Integration_Runtime"])
        except Exception as e:
            # st.warning(f"Error calculating IR count: {e}")
            integration_runtimes = 0
        ...
        # Option: premium KPI cards vs compact metric-cards
        use_premium_tiles = True
        try:
            use_premium_tiles = st.checkbox("Use premium KPI cards (rounded tiles)", value=True)
        except Exception:
            use_premium_tiles = True
        ...
        if use_premium_tiles:
            # Build data and render using premium tiles grid
            tiles_data = [
                {"icon": "📦", "value": pipelines, "label": "Pipelines", "variant": "purple", "subtitle": tile_descriptions.get("Pipelines")},
                {"icon": "🌊", "value": dataflows, "label": "DataFlows", "variant": "pink", "subtitle": tile_descriptions.get("DataFlows")},
                {"icon": "📁", "value": datasets, "label": "Datasets", "variant": "blue", "subtitle": tile_descriptions.get("Datasets")},
                {"icon": "🔗", "value": linked_services, "label": "Linked Svc", "variant": "indigo", "subtitle": "Total Linked Services"},
                {"icon": "⚙️", "value": integration_runtimes, "label": "IRs", "variant": "teal", "subtitle": "Total Integration Runtimes"},
                {"icon": "⏰", "value": triggers, "label": "Triggers", "variant": "green", "subtitle": tile_descriptions.get("Triggers")},
                {"icon": "🕸", "value": dependencies, "label": "Dependencies", "variant": "purple", "subtitle": tile_descriptions.get("Dependencies")},
                {"icon": "💚", "value": f"{health_score}%", "label": "Health", "variant": "blue", "subtitle": tile_descriptions.get("Health")},
                {"icon": "⚠️" if orphaned>0 else "✅", "value": orphaned, "label": "Orphaned", "variant": "fire" if orphaned>0 else "success", "subtitle": tile_descriptions.get("Orphaned")},
            ]
            try:
                render_premium_tiles_grid(tiles_data, columns=9, animated=True)
            except Exception:
                # Fallback to compact layout if premium rendering fails
                cols = st.columns(7)
                for col, t in zip(cols, tiles_data):
                    with col:
                        st.markdown(
                            f"""
                            <div class="metric-card gradient-purple" title="{t.get('subtitle','')}">
                                <div style="font-size: 1.6em;">{t.get('icon','')}</div>
                                <div class="metric-label">{t.get('label')}</div>
                                <div class="metric-value">{format_number(t.get('value')) if not isinstance(t.get('value'), str) else t.get('value')}</div>
                            </div>
                            """,
                            unsafe_allow_html=True,
                        )
        ...
        with st.expander("🔎 Lineage & Details (expand for dataset/tables breakdown and Verify)", expanded=False):
            ...
            with c1:
                ...
                try:
                    if not src_counts.empty:
                        ...
                        safe_plotly(fig_src, use_container_width=True)
                    else:
                ...
                try:
                    if not tgt_counts.empty:
                        ...
                        safe_plotly(fig_tgt, use_container_width=True)
                    else:
            ...
            with c2:
                ...
                if not sankey_df.empty:
                    # Calculate link counts for all pairs
                    link_counts = sankey_df.groupby(["Source", "Sink"]).size().reset_index(name="count").sort_values("count", ascending=False)
                    
                    # NEW LOGIC: Filter by TOP FLOWS (strongest links) first
                    # This ensures we see the most important connections, not just popular nodes
                    top_links = link_counts.head(20)  # Show top 20 strongest flows
                    
                    if top_links.empty:
                         st.info("Not enough data for Sankey")
                    else:
                        # Get unique nodes from the top links
                        nodes = list(set(top_links["Source"].unique().tolist() + top_links["Sink"].unique().tolist()))
                        node_idx = {n: i for i, n in enumerate(nodes)}
                        
                        source_idx = [node_idx[s] for s in top_links["Source"]]
                        target_idx = [node_idx[t] for t in top_links["Sink"]]
                        values = top_links["count"].tolist()
                        
                        # Use premium colors
                        node_colors = px.colors.qualitative.Dark24
                        if len(nodes) > len(node_colors):
                            node_colors = node_colors * (len(nodes) // len(node_colors) + 1)
                        ...
                        sankey_fig.update_layout(
                            ...
                            font=dict(size=12, color="gray"), # Default fallback
                            paper_bgcolor='rgba(0,0,0,0)',
                            plot_bgcolor='rgba(0,0,0,0)'
                        )
                        
                        # Apply theme specific text color if available in session state
                        if st.session_state.get("theme") == "light":
                            sankey_fig.update_layout(font=dict(color="#0f172a"))
                        else:
                            sankey_fig.update_layout(font=dict(color="#f1f5f9"))
                            
                        safe_plotly(sankey_fig, use_container_width=True)
                else:
    ...
    def render_overview_tab(self):
        ...
        # Retrieve necessary dataframes locally
        pipeline_df = safe_get_dataframe("ImpactAnalysis", "PipelineAnalysis", "Pipeline_Analysis", "Pipelines")
        lineage_df = safe_get_dataframe("DataLineage", "Data_Lineage")
        pipeline_pipeline_df = safe_get_dataframe("Pipeline_Pipeline", "PipelinePipeline")
        dflow_lineage = safe_get_dataframe("DataFlowLineage", "DataFlow_Lineage")
        ...
        # --- EXECUTIVE SUMMARY ("The Best Thing") ---
        try:
            st.markdown("#### 🚀 Executive Summary")
            
            # Calculate Metrics
            total_pipelines = len(pipeline_df) if not pipeline_df.empty else 1
            orphaned_count = len(pipeline_df[pipeline_df['IsOrphaned'].isin([True, 'Yes', 1])]) if not pipeline_df.empty and 'IsOrphaned' in pipeline_df.columns else 0
            
            # Health Score (Simple heuristic: 100 - % orphaned)
            health_score = max(0, 100 - int((orphaned_count / total_pipelines) * 100))
            
            # Top Data Asset (Most connected node in lineage)
            top_asset = "N/A"
            if not lineage_df.empty:
                # Combine source and sink to find most frequent
                all_nodes = pd.concat([lineage_df['Source'], lineage_df['Sink']])
                if not all_nodes.empty:
                    top_asset = all_nodes.mode().iloc[0]

            # Critical Pipeline (Most dependencies)
            critical_pipeline = "N/A"
            if not pipeline_pipeline_df.empty:
                # Count occurrences in 'from_pipeline' (downstream dependencies)
                deps = pipeline_pipeline_df['from_pipeline'].value_counts()
                if not deps.empty:
                    critical_pipeline = deps.index[0]
            ...
            with exec_c1:
                render_metric_card_premium(
                    ...
                    "Operational Score",
                    "activity", 
                    "green" if health_score > 80 else "orange"
                )
            
            with exec_c2:
                render_metric_card_premium(
                    "Critical Pipeline", 
                    critical_pipeline[:15] + "..." if len(critical_pipeline) > 15 else critical_pipeline,
                    "Most Dependencies",
                    "git-commit", 
                    "purple"
                )
            ...
            with exec_c3:
                render_metric_card_premium(
                    "Top Data Asset", 
                    top_asset[:15] + "..." if len(top_asset) > 15 else top_asset,
                    "Most Connected",
                    "database", 
                    "blue"
                )
            ...
            st.error(f"Could not load Executive Summary: {e}")
        # ---------------------------------------------
        ...
        # Row 4: Sankey Diagram (Top Flows)
        st.markdown("---")
        c1, c2 = st.columns([1, 20]) # Use mostly full width or adjust as needed. Original was inside render_enhanced_metrics with c2.
        # Actually, let's keep it consistent with the previous layout or put it in a new full-width section.
        # The previous code had it in 'render_enhanced_metrics' which is separate.
        # Wait, I am editing 'render_overview_tab'.
        # The Sankey code I pasted before was inside 'render_enhanced_metrics' (lines 5630+).
        # But I am editing 'render_overview_tab' (lines 5682+).
        
        # Ah, I see. The Sankey diagram was originally in `render_enhanced_metrics` (which is called "Factory Metrics Dashboard").
        # The Executive Summary is in `render_overview_tab` (which is called "Factory Overview Dashboard").
        
        # I need to fix the Executive Summary in `render_overview_tab`.
        # AND I need to fix the Sankey diagram in `render_enhanced_metrics`.
        
        # This tool call is targeting `render_overview_tab` (lines 5682+).
        # So I will ONLY fix the Executive Summary here.
        # I will fix the Sankey diagram in a separate call or if I can reach it.
        
        # Wait, the previous `replace_file_content` (Step 284) modified `render_enhanced_metrics` to add the Top Flows logic.
        # But `render_enhanced_metrics` ALSO uses `lineage_df` and `dflow_lineage`.
        # In `render_enhanced_metrics`, `lineage_df` and `dflow_lineage` ARE defined locally (lines 5185 and 5223).
        # So the Sankey diagram in `render_enhanced_metrics` SHOULD work, provided `lineage_df` and `dflow_lineage` are correctly populated.
        
        # The error I saw was "Could not load Executive Summary".
        # So I just need to fix `render_overview_tab`.
        
        # I will proceed with fixing `render_overview_tab`.
    ...
    def render_resource_summary(self):
        ...
        fig = go.Figure(
            go.Treemap(
                ...
                marker=dict(colorscale="Viridis", line=dict(width=2, color=theme_line_color())),
    ...
    def render_2d_network(self, G, pos: dict, show_labels: bool):
        ...
        safe_plotly(fig, use_container_width=True)
        ...
        try:
            fig = go.Figure(data=[go.Sankey(
                node=dict(
                    pad=15,
                    thickness=20,
                    line=dict(color=theme_line_color(), width=2),
                    label=labels,
                    color=[
