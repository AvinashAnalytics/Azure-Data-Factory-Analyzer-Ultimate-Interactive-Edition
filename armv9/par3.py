"""
═══════════════════════════════════════════════════════════════════════════════
ULTIMATE Enterprise Azure Data Factory Analyzer v9.1 - FIXED VERSION
═══════════════════════════════════════════════════════════════════════════════
...
🔴 #2: SQL CTE pattern - Fixed table extraction
🔴 #3: Activity dependencies - Added dependency tracking
🔴 #4: Orphaned triggers - Added state check
🟡 #5: Pipeline analysis - Populated comprehensive analysis
🟡 #6: Column widths - Auto-adjustment added
🟡 #7: Circular dependencies - Detection added
🟡 #8: Impact analysis - Multi-level recursive
🟡 #9: Sequence numbering - Simplified logic
🟡 #10: Max depth - Protection added
🟢 #11: Column parsing - String literal handling
🟢 #12: Sheet sanitization - Excel-safe names
🟢 #13: Impact sorting - NaN handling
🟢 #14: Formatting - Conditional formatting added
🟢 #15: Usage stats - Resource usage tracking

Author: Enterprise ADF Team
Date: 2024
Version: 9.1 - Complete Integration + Bug Fixes
═══════════════════════════════════════════════════════════════════════════════
"""
...
class UltimateEnterpriseADFAnalyzer:
    """
    Complete Enterprise ADF Analyzer - Production Ready (v9.1 - FIXED)
    ...
    - ✅ Impact analysis (multi-level recursive)
    ...
    Bug Fixes:
    - ✅ Fixed dataset location extraction (schema.table)
    - ✅ Fixed SQL table parsing (CTE handling)
    - ✅ Fixed activity-to-activity dependencies
    - ✅ Fixed orphaned trigger detection (state check)
    - ✅ Added pipeline analysis sheet
    - ✅ Added column width auto-adjustment
    - ✅ Added circular dependency detection
    - ✅ Enhanced impact analysis (multi-level)
    - ✅ Simplified sequence numbering
    - ✅ Added max depth protection
    - ✅ Added sheet name sanitization
    ...
    MAX_ACTIVITY_DEPTH = 20  # ✅ FIX #10: Maximum nesting depth
    ...
    def __init__(self, json_path: str, enable_discovery: bool = True):
        ...
        self.results = {
            ...
            'pipelines': [],
            'pipeline_analysis': [],  # ✅ FIX #5: Now populated
            'activities': [],
            'activity_count': [],
            'activity_execution_order': [],  # ✅ FIX #3: Activity dependencies
            ...
            # Analysis sheets
            'data_lineage': [],
            'impact_analysis': [],  # ✅ FIX #8: Enhanced multi-level
            'circular_dependencies': [],  # ✅ FIX #7: New sheet
            
            # Orphaned resource detection
            'orphaned_pipelines': [],
            'orphaned_datasets': [],
            'orphaned_linked_services': [],
            'orphaned_triggers': [],  # ✅ FIX #4: Enhanced detection
            
            # ✅ FIX #15: Usage statistics
            'dataset_usage': [],
            'linkedservice_usage': [],
        ...
        self.dependencies = {
            ...
            'activity_to_activity': [],        # ✅ FIX #3: Activity → Activity
        ...
        print(f"🚀 Ultimate Enterprise ADF Analyzer v9.1 (FIXED)")
    ...
    def sanitize_sheet_name(self, name: str) -> str:
        r"""
        ✅ FIX #12: Sanitize sheet name for Excel compatibility
    ...
    def run(self) -> bool:
        ...
        5.5. Detect circular dependencies (✅ FIX #7)
        6. Extract relationships & lineage
        7. Detect orphaned resources
        8. Analyze impact (✅ FIX #8: Multi-level)
        9. Calculate statistics
        9.5. Build pipeline analysis (✅ FIX #5)
        9.6. Calculate resource usage (✅ FIX #15)
        10. Export to Excel
        11. Print summary
        ...
        """
        print("\n" + "="*80)
        print("ENTERPRISE ADF ANALYSIS - COMPLETE INTEGRATION (v9.1 FIXED)")
        print("="*80)
        ...
        try:
            ...
            # Phase 5.5: ✅ FIX #7: Circular dependencies
            ...
            # Phase 8: Impact analysis (✅ FIX #8: Enhanced)
            ...
            # Phase 9.5: ✅ FIX #5: Pipeline analysis
            print("\n📊 Phase 9.5/12: Building pipeline analysis...")
            self.build_pipeline_analysis()
            
            # Phase 9.6: ✅ FIX #15: Resource usage
            print("\n📊 Phase 9.6/12: Calculating resource usage...")
    ...
    def parse_dataset(self, resource: dict):
        ...
        - Integration Runtime (from LinkedService)
    ...
    def parse_pipeline(self, resource: dict):
        ...
        try:
            ...
            # Parse all activities using fixed nested parsing
    ...
    def parse_nested_activities(self, activities: List[dict], pipeline: str, 
        ...
        Args:
            ...
            depth: Current nesting depth (0=root, 1=first level, etc.)
            start_seq: Starting sequence number
        
        Returns:
            Next available sequence number (ready to use, no adjustment needed)
        """
        
        # ✅ FIX #10: Check maximum depth
        if depth > self.MAX_ACTIVITY_DEPTH:
            ...
            # Return current sequence to continue parsing other activities
            return start_seq
            ...
            try:
                ...
                if activity_type == 'ForEach':
                    ...
                    if isinstance(nested_acts, list) and nested_acts:
                        # ✅ FIX #9: Use returned value directly (no -1/+1 confusion)
                ...
                current_seq += 1  # Skip this sequence on error
        
        # Return next available sequence (ready to use, no adjustment)
        return current_seq
    ...
    def parse_activity(self, activity: dict, pipeline: str, seq: int, parent: str = '', depth: int = 0):
        """
        ...
        ✅ Better table/column parsing (✅ FIX #2)
        ...
        # Extract SQL with enhanced parsing (✅ FIX #2)
    ...
    def extract_integration_runtime_from_activity(self, activity: dict, type_props: dict) -> str:
        """
        ✅ Extract Integration Runtime from activity
    ...
    def extract_sql_enhanced(self, activity: dict, type_props: dict, rec: dict):
        ...
        ✅ Better table name extraction (fixed CTE handling)
        ...
        if sql_text:
            ...
            # ✅ FIX #2: Parse SQL with fixed CTE handling
    ...
    def parse_sql_for_tables_and_columns(self, sql: str) -> Tuple[List[str], List[str]]:
        ...
        - String literal handling (doesn't break on commas/parens in strings)
        """
        tables = []
        columns = []
        ...
        # ═══════════════════════════════════════════════════════════════════
        table_patterns = [
            # ✅ REMOVED: r'WITH\s+(\w+)\s+AS' - was extracting CTE aliases
        ...
        # ═══════════════════════════════════════════════════════════════════
        # ✅ FIX #11: Column extraction with string literal handling
        # ═══════════════════════════════════════════════════════════════════
        ...
        if select_match:
            ...
            if '*' in select_part:
                ...
                # Split by comma (handling nested functions AND string literals)
                parts = []
                depth = 0
                current = []
                in_string = False
        ...
        if isinstance(depends_on, list):
            for dep in depends_on:
                if isinstance(dep, dict):
                    ...
                    if dep_activity:
                        # ✅ FIX #3: Store in dependencies dictionary
    ...
    def extract_all_dependencies(self):
        ...
        # ═══════════════════════════════════════════════════════════════════
        # 3-6. Activity-level Dependencies (✅ FIX #3: Now includes activity_to_activity)
        # ═══════════════════════════════════════════════════════════════════
        ...
        # ═══════════════════════════════════════════════════════════════════
        # Build Dependency Graph (✅ FIX #3: Now includes activity→activity)
        # ═══════════════════════════════════════════════════════════════════
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # CIRCULAR DEPENDENCY DETECTION - ✅ FIX #7
    # ═══════════════════════════════════════════════════════════════════════
    
    def detect_circular_dependencies(self):
        """
        ✅ FIX #7: Detect circular dependencies in the dependency graph
        ...
        for cycle_info in cycles_found:
            ...
            normalized = tuple(sorted(cycle_parts[:-1]))
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # ORPHANED RESOURCE DETECTION - ✅ FIX #4
    # ═══════════════════════════════════════════════════════════════════════
    ...
    def detect_orphaned_resources(self):
        ...
        - ✅ NEW: Deduplicates trigger-pipeline pairs to show unique triggers
        """
        ...
        # ═══════════════════════════════════════════════════════════════════
        # ✅ FIX: Enhanced Trigger Detection with Deduplication
        # ═══════════════════════════════════════════════════════════════════
        orphaned_trigger_set = set()  # Track unique triggers
        orphaned_trigger_info = {}     # Store trigger details
        ...
        # ✅ Store unique orphaned triggers
        self.results['orphaned_triggers'] = list(orphaned_trigger_info.values())
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # IMPACT ANALYSIS - ✅ FIX #8 (Multi-level Recursive)
    # ═══════════════════════════════════════════════════════════════════════
    
    def analyze_impact(self):
        """
        ✅ FIX #8: Enhanced multi-level recursive impact analysis
        ✅ FIX #16: Focus on FUNCTIONAL dependencies, not ARM structural
        
        FIXED:
        - Now uses only triggers and ExecutePipeline for upstream
        ...
        for pipeline_name in self.resources['pipelines'].keys():
            ...
            if transitive_upstream:
                ...
                for depth in sorted([k for k in transitive_upstream.keys() if isinstance(k, int)]):
                    ...
                    if resources:
                        parts.append(f"L{depth}:{','.join(list(resources)[:3])}")
            ...
            self.results['impact_analysis'].append({
                ...
                'DirectUpstreamTriggers': ', '.join(upstream_triggers) if upstream_triggers else 'None',
                'DirectUpstreamTriggerCount': len(upstream_triggers),
                'DirectUpstreamPipelines': ', '.join(direct_upstream_pipelines) if direct_upstream_pipelines else 'None',
                ...
                'DirectDownstreamPipelines': ', '.join(direct_downstream_pipelines) if direct_downstream_pipelines else 'None',
    ...
    def _get_downstream_recursive_functional(self, pipeline_name: str, max_depth: int = 5,
        ...
        # Get functional downstream only
    ...
    def calculate_activity_counts(self):
        ...
        print(f"  ✅ Activity count summary: {len(self.results['activity_count'])-1} activity types")
            # ═══════════════════════════════════════════════════════════════════════
    # PIPELINE ANALYSIS - ✅ FIX #5
    # ═══════════════════════════════════════════════════════════════════════
    
    def build_pipeline_analysis(self):
        """
        ✅ FIX #5: Build comprehensive pipeline analysis sheet
        
        For each pipeline, provides:
        - Activity counts by type
        - Dependency counts (triggers, dataflows, datasets)
        - SQL/SP usage flags
        - Complexity score
        """
        ...
        for pipeline_name, pipeline_resource in self.resources['pipelines'].items():
            ...
            self.results['pipeline_analysis'].append({
                ...
                'StoredProcActivities': activity_counts.get('SqlServerStoredProcedure', 0),
                ...
                'Triggers': ', '.join(triggers[:5]) + (f' (+{len(triggers)-5})' if len(triggers) > 5 else ''),
                'UpstreamPipelines': len(upstream_pipelines),
                'UpstreamPipelineNames': ', '.join(upstream_pipelines[:3]) + (f' (+{len(upstream_pipelines)-3})' if len(upstream_pipelines) > 3 else ''),
                'DownstreamPipelines': len(downstream_pipelines),
                'DownstreamPipelineNames': ', '.join(downstream_pipelines[:3]) + (f' (+{len(downstream_pipelines)-3})' if len(downstream_pipelines) > 3 else ''),
                'DataFlowCount': len(dataflows),
                'DataFlowNames': ', '.join(dataflows[:3]) + (f' (+{len(dataflows)-3})' if len(dataflows) > 3 else ''),
    ...
    def calculate_resource_usage_statistics(self):
        ...
        for dataset, count in dataset_usage.most_common():
            ...
            self.results['dataset_usage'].append({
                ...
                'Consumers': ', '.join(consumers[:5]) + (f' (+{len(consumers)-5})' if len(consumers) > 5 else ''),
    ...
    # ═══════════════════════════════════════════════════════════════════════
    # EXCEL EXPORT - WITH ALL FIXES (#6, #12, #13, #14)
    # ═══════════════════════════════════════════════════════════════════════
    ...
    def export_to_excel(self):
        ...
        Applied Fixes:
        ✅ FIX #6: Auto-adjust column widths
        ✅ FIX #12: Sanitize sheet names (Excel-safe)
        ✅ FIX #13: Handle NaN in sorting
        ✅ FIX #14: Conditional formatting
        """
        ...
        try:
            with pd.ExcelWriter(excel_file, engine='openpyxl') as writer:
                ...
                # ✅ FIX #14: Apply conditional formatting
            ...
            # Auto-copy to Streamlit (if configured)
            ...
            if config_file.exists():
                try:
                    ...
                    if config.get('auto_copy', False):
                        ...
                        # Validate path
                        if '..' in str(streamlit_path):
                            print(f"  ⚠️  Warning: Streamlit path contains '..' - skipping auto-copy")
                        else:
                            # Create directory
                            try:
                                ...
                                print(f"  ❌ Permission denied: Cannot create {streamlit_path}")
                            except Exception as e:
                                print(f"  ❌ Auto-copy failed: {e}")
                
                except json.JSONDecodeError as e:
                    print(f"  ❌ Invalid JSON in streamlit_config.json: {e}")
                except Exception as e:
    ...
    def _write_core_data_sheets(self, writer):
        ...
        # ✅ FIX #5: Include pipeline_analysis
        core_sheets = [
            ('Pipelines', self.results['pipelines']),
            ('PipelineAnalysis', self.results['pipeline_analysis']),  # ✅ NEW
    ...
    def _write_analysis_sheets(self, writer):
        ...
        analysis_sheets = [
            ...
            ('CircularDependencies', self.results['circular_dependencies']),  # ✅ FIX #7
        ]
        ...
        for sheet_name, data in analysis_sheets:
            if data:
                ...
                # ✅ FIX #13: Sort with NaN handling
                if sheet_name == 'CircularDependencies':
                ...
                df.to_excel(writer, sheet_name=safe_name, index=False)
                self._auto_adjust_columns(writer, safe_name)  # ✅ FIX #6
    ...
    def _write_sheet_with_auto_split(self, writer, sheet_name: str, data: List[Dict]):
        ...
        ✅ FIX #6: Auto-adjust columns
        ✅ FIX #12: Sanitize sheet names
        """
    ...
    def _auto_adjust_columns(self, writer, sheet_name: str):
        """
        ✅ FIX #6: Auto-adjust column widths based on content
        """
        try:
    ...
    def _apply_conditional_formatting(self, writer):
        """
        ✅ FIX #14: Apply conditional formatting to Excel sheets
        """
        try:
    ...
    def print_summary(self):
        ...
        """
        print("\n" + "="*80)
        print("ENTERPRISE ADF ANALYSIS COMPLETE (v9.1 FIXED)")
        print("="*80)
...
╔══════════════════════════════════════════════════════════════════════════════╗
║                                                                              ║
║   Ultimate Enterprise ADF Analyzer v9.1 - FIXED & PRODUCTION READY          ║
║                                                                              ║
║  ✅ ALL 15 BUGS FIXED                                                        ║
║  ✅ ALL Meeting Requirements Met                                             ║
║  ✅ Production Ready                                                         ║
║                                                                              ║
║  Bug Fixes Applied:                                                          ║
║  🔴 #1-4: Critical bugs (dataset location, SQL, dependencies, triggers)     ║
║  🟡 #5-10: Important bugs (pipeline analysis, depth, impact, etc.)          ║
║  🟢 #11-15: Enhancements (formatting, usage stats, etc.)                    ║
║                                                                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
    """)
    ...
    if len(sys.argv) < 2:
        print("""
USAGE:
  python adf_analyzer_v9_1_fixed.py <template.json> [--no-discovery]
...
BUG FIXES IN v9.1:
  🔴 CRITICAL:
    #1: Dataset location - Fixed schema.table combination
    #2: SQL CTE pattern - Fixed table extraction from CTEs
    #3: Activity dependencies - Added activity→activity tracking
    #4: Orphaned triggers - Added state checking (Stopped triggers detected)
  
  🟡 IMPORTANT:
    #5: Pipeline analysis - Now populated with comprehensive metrics
    #6: Column widths - Auto-adjustment added
    #7: Circular dependencies - Detection added
    #8: Impact analysis - Multi-level recursive (5 levels deep)
    #9: Sequence numbering - Simplified logic
    #10: Max depth - Protection added (limit: 20 levels)
  ...
  🟢 ENHANCEMENTS:
    ...
    #13: Impact sorting - NaN handling
    #14: Formatting - Conditional formatting added
    #15: Usage stats - Resource usage tracking added
...
EXCEL SHEETS (25+):
  ...
  • Pipelines - All pipelines
  • PipelineAnalysis - ⭐ Comprehensive pipeline metrics (NEW)
  • Activities - All activities (auto-split if >500k rows)
  • ActivityCount - Activity usage summary
  • ImpactAnalysis - ⭐ Multi-level impact analysis (ENHANCED)
  • CircularDependencies - ⭐ Circular dependency detection (NEW)
  • OrphanedPipelines - Unused pipelines
  • OrphanedDatasets - Unused datasets
  • OrphanedTriggers - Stopped/broken triggers (ENHANCED)
  • DatasetUsage - ⭐ Dataset usage statistics (NEW)
  • LinkedServiceUsage - ⭐ LinkedService usage statistics (NEW)
  • DataLineage - Complete source→sink lineage
  • And 15+ more sheets...

EXAMPLES:
  # Standard analysis with discovery
  python adf_analyzer_v9_1_fixed.py factory_arm_template.json

  # Fast analysis without discovery
  python adf_analyzer_v9_1_fixed.py factory_arm_template.json --no-discovery

  # Large template (350+ pipelines)
  python adf_analyzer_v9_1_fixed.py large_factory.json
  ...
  For issues, check the Errors sheet in the output Excel file.
    ...
    try:
        ...
        if success:
            ...
            print(f"  6. Use: adf_analysis_latest.xlsx in Streamlit dashboard")
            sys.exit(0)
        else:
...
╔══════════════════════════════════════════════════════════════════════════════╗
║                                                                              ║
║  ULTIMATE ENTERPRISE ADF ANALYZER v9.1 - COMPLETE & FIXED                   ║
║                                                                              ║
║  Total Lines: ~2500+                                                         ║
║  All Meeting Requirements: ✅ MET                                             ║
║  All Critical Bugs: ✅ FIXED                                                  ║
...
║  ├─ Part 2: Pattern Discovery & Dataset Location (FIX #1)                   ║
║  ├─ Part 3: Resource Parsing (IR, LS, Dataset, DataFlow)                    ║
║  ├─ Part 4: Pipeline & Activity Parsing (FIX #2, #3, #9, #10, #11)         ║
║  ├─ Part 5: Triggers, Dependencies, Impact Analysis (FIX #4, #7, #8)       ║
║  └─ Part 6: Analysis, Statistics, Export & Main (FIX #5, #6, #12-15)       ║
║                                                                              ║
║  Usage:                                                                      ║
║    python adf_analyzer_v9_1_fixed.py factory_arm_template.json              ║
║                                                                              ║
...
║                                                                              ║
║  Bug Fixes:                                                                  ║
║    🔴 Critical (4): Dataset location, SQL CTE, Activity deps, Trigger state ║
║    🟡 Important (6): Pipeline analysis, Column width, Circular deps, etc.   ║
║    🟢 Enhancement (5): Formatting, Usage stats, String handling, etc.       ║
║                                                                              ║
║  Key Features:                                                               ║
║    • 25+ Excel sheets with complete analysis                                ║
║    • Auto-split for sheets >500k rows                                       ║
║    • Multi-level impact analysis (5 levels deep)                            ║
║    • Circular dependency detection                                          ║
║    • Orphaned resource detection (all types)                                ║
