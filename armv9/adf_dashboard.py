...
def safe_get_dataframe(sheet_name: str, *alternative_names: str) -> pd.DataFrame:
    ...
    """
    # Prefer normalized sheet map if available
    sheet_map = st.session_state.get("sheet_map")
    ...
    def try_get(key):
        ...
        if sheet_map and key in sheet_map:
            actual = sheet_map.get(key)
            if actual in data and isinstance(data[actual], pd.DataFrame):
                return data[actual]
        return None

    # Try primary name and canonical variants
    df = try_get(sheet_name)
    if df is not None:
        return df
    ...
    # Try normalized matching: strip non-alnum and lowercase
    def norm_key(s: str) -> str:
        return re.sub(r"[^0-9a-z]+", "", str(s).lower())

    requested = [sheet_name] + list(alternative_names)
    requested_norm = {norm_key(r): r for r in requested if r}

    # check sheet_map keys
    if sheet_map:
        for k, actual in sheet_map.items():
            if norm_key(k) in requested_norm:
                df = data.get(actual)
                if isinstance(df, pd.DataFrame):
                    return df

    # Lastly, try to find any sheet whose normalized name matches requested_norm
    ...
    # If still not found, try hyperlink_map (display text -> sheet name)
    hyperlink_map = st.session_state.get("hyperlink_map", {})
    if hyperlink_map:
        for rq in requested:
            if not rq:
                continue
            h = hyperlink_map.get(rq.lower())
            if h:
                # If h is internal sheet ref, try to return that sheet
                # If the hyperlink was external URL, skip
                if isinstance(h, str) and not h.lower().startswith(('http://', 'https://', 'mailto:')):
                    # h may be sheet name
                    if h in data and isinstance(data[h], pd.DataFrame):
                        return data[h]
                    # try normalized match
                    for k2 in data.keys():
                        if norm_key(k2) == norm_key(h):
                            return data[k2]
...
def get_summary_metric(metric_name: str, default: Any = 0) -> Any:
    ...
    try:
        ...
        # Normalize numeric-like strings (remove commas, percent signs)
        if isinstance(val, str):
            ...
            try:
                ...
                # not a pure number, return original string
                return val
...
class ADF_Dashboard:
    ...
    def render_sidebar_stats(self):
        ...
        # Use robust count retrieval with sheet fallbacks
        pipelines = get_count_with_fallback(
            "Pipelines", ["ImpactAnalysis", "PipelineAnalysis", "Pipeline_Analysis", "Pipelines"]
        )
        dataflows = get_count_with_fallback("DataFlows", ["DataFlows", "DataFlowLineage", "DataFlow_Summary"]) 
    ...
    def load_excel_file(self, uploaded_file):
        ...
        try:
            with st.spinner("🔄 Loading analysis file..."):
                ...
                # Attempt to extract hyperlinks from the workbook (so dashboard can respect internal nav links)
                hyperlink_map = {}
                try:
                    if 'HAS_OPENPYXL' in globals() and HAS_OPENPYXL:
                        try:
                            # Reset file pointer and load with openpyxl to extract hyperlinks
                        ...
                        try:
                            ...
                            wb = load_workbook(uploaded_file, read_only=True, keep_links=True)
                                ...
                                for row in ws.iter_rows(values_only=False):
                                    for cell in row:
                                        try:
                                            ...
                                            if hl:
                                                # hl can be a Hyperlink object or a string
                                                target = None
                                                if hasattr(hl, 'target'):
                                                    target = hl.target
                                                else:
                                                    target = str(hl)

                                                display = str(cell.value) if cell.value is not None else ''
                                                # Internal link format: #'Sheet Name'!A1 or #SheetName!A1
                                                if isinstance(target, str) and target.startswith('#'):
                                                    # strip leading '#'
                                                    t = target.lstrip('#')
                                                    # split at !
                                                    parts = t.split('!')
                                                    sheet_ref = parts[0].strip().strip("'")
                                                    hyperlink_map.setdefault(display.lower(), sheet_ref)
                                                else:
                                                    # external link
                                                    hyperlink_map.setdefault(display.lower(), target)
                                        except Exception:
                                            continue
                        except Exception:
                            # openpyxl failed; ignore hyperlink extraction
                            hyperlink_map = {}

                except Exception:
                    hyperlink_map = {}

                # Normalize and merge sheets where necessary (handles variants like P1/P2 and inconsistent names)
                sheet_map = {}
                normalized_data = dict(data)  # copy

                def normalize_name(name: str) -> str:
                    return re.sub(r"[^0-9a-z]+", "", str(name).lower())

                # Merge parts like SheetName_P1 / _P2 into SheetName
                part_re = re.compile(r"^(?P<base>.+?)_p(\d+)$", re.IGNORECASE)
                groups = defaultdict(list)
                for original in list(data.keys()):
                    m = part_re.match(original)
                    if m:
                        base = m.group('base')
                        groups[base].append(original)

                for base, parts in groups.items():
                    # concatenate parts in sort order
                    parts_sorted = sorted(parts)
                    dfs = [data[p] for p in parts_sorted if isinstance(data.get(p), pd.DataFrame)]
                    if dfs:
                        merged = pd.concat(dfs, ignore_index=True, sort=False)
                        # Replace or add merged key using canonical base name
                        normalized_data[base] = merged
                        # keep mapping from canonical names to actual merged key
                        sheet_map[base] = base

                # Build sheet_map for common canonical targets
                for actual in list(normalized_data.keys()):
                    n = normalize_name(actual)
                    # map common canonical names
                    if n in ["impactanalysis", "pipelineanalysis", "pipeline_analysis", "pipelines"]:
                        sheet_map.setdefault("ImpactAnalysis", actual)
                        sheet_map.setdefault("Pipelines", actual)
                    if n in ["dataflows", "dataflowlineage", "dataflow_summary"]:
                        sheet_map.setdefault("DataFlows", actual)
                    if n in ["activitycount", "activity_count"]:
                        sheet_map.setdefault("ActivityCount", actual)
                    if n in ["summary"]:
                        sheet_map.setdefault("Summary", actual)
                    if n in ["triggerdetails", "triggers"]:
                        sheet_map.setdefault("TriggerDetails", actual)

                # Store both raw and normalized structures
                # Keep raw copy for debugging and keep `excel_data` as the
                # normalized/merged version so later lookups (via sheet_map)
                # resolve correctly. Previously we accidentally overwrote
                # `excel_data` with the raw `data` which removed merged
                # sheet keys and caused counts/charts to return empty.
                st.session_state.excel_data_raw = data
                st.session_state.excel_data = normalized_data
                st.session_state.sheet_map = sheet_map
                st.session_state.hyperlink_map = hyperlink_map

                status_text.text("💾 Storing data...")
                progress_bar.progress(70)

                # Keep filename and do NOT overwrite the normalized excel_data
    ...
    def extract_metadata(self):
        ...
        # Populate some derived summary metrics when Summary sheet is missing or incomplete
        summary_map = metadata.get("summary", {}) or {}

        def ensure_metric(name: str, fallback_sheets: List[str]):
            v = summary_map.get(name)
            if v is None or v == "" or (isinstance(v, (int, float)) and int(v) == 0):
                # compute fallback
                df = None
                for s in fallback_sheets:
                    df = safe_get_dataframe(s)
                ...
                if df is not None and not df.empty:
                    summary_map[name] = len(df)

        ensure_metric("Pipelines", ["ImpactAnalysis", "PipelineAnalysis", "Pipeline_Analysis", "Pipelines"])
        ensure_metric("DataFlows", ["DataFlows", "DataFlowLineage", "DataFlow_Summary"])
        ensure_metric("Datasets", ["Datasets"])
        ensure_metric("Triggers", ["TriggerDetails", "Triggers"])
        ensure_metric("Total Dependencies", ["ActivityExecutionOrder", "DataLineage", "Pipeline_Pipeline", "Pipeline_DataFlow", "TriggerDetails"])
        ensure_metric("Orphaned Pipelines", ["OrphanedPipelines", "Orphaned_Pipelines"])

        metadata["summary"] = summary_map
    ...
    def show_load_summary(self):
        ...
        with col3:
            ...
            # Fallback to sheet count if Summary metric is missing or zero
            if (not pipelines or pipelines == 0) and (
                "PipelineAnalysis" in st.session_state.excel_data
                or "Pipelines" in st.session_state.excel_data
            ):
                p_df = safe_get_dataframe("PipelineAnalysis", "Pipelines")
                pipelines = len(p_df) if not p_df.empty else pipelines

            st.metric("Pipelines", format_number(pipelines))

        with col4:
            dataflows = get_summary_metric("DataFlows", 0)
            if (not dataflows or dataflows == 0) and (
                "DataFlows" in st.session_state.excel_data
                or "DataFlowLineage" in st.session_state.excel_data
            ):
                d_df = safe_get_dataframe("DataFlows", "DataFlowLineage")
                dataflows = len(d_df) if not d_df.empty else dataflows
    ...
    def render_welcome_screen(self):
        ...
        with col2:
            st.markdown(
                """
            <div class="info-card fade-in" style="text-align: center; padding: 3rem; margin-top: 2rem;">
                <div style="font-size: 4em; margin-bottom: 20px;">🏭</div>
                <h2 style="color: #667eea; margin-bottom: 10px;">Welcome to ADF Analyzer v10.1!</h2>
                ...
                <div style="background: linear-gradient(135deg, #667eea15 0%, #764ba215 100%); 
                    ...
                    <div style="text-align: left; display: inline-block; max-width: 600px;">
                        <p>🌐 <strong>Network Visualizations</strong> - Interactive 2D & 3D dependency graphs</p>
                        <p>📊 <strong>Advanced Charts</strong> - 15+ chart types for comprehensive analysis</p>
                        <p>🎯 <strong>Impact Analysis</strong> - Understand change impact before making it</p>
                        <p>⚠️ <strong>Orphan Detection</strong> - Find unused resources automatically</p>
                        <p>🔍 <strong>Smart Search</strong> - Quickly find any resource</p>
                        <p>📈 <strong>Statistics</strong> - Activity distribution and usage metrics</p>
                        <p>📥 <strong>Export</strong> - Download filtered data and reports</p>
                    </div>
                </div>
                
                <p style="color: #999; margin-top: 30px;">
                    👈 Use the sidebar to upload your analysis file or load sample data
                </p>
            </div>
            """,
                unsafe_allow_html=True,
    ...
    def render_enhanced_metrics(self):
        ...
        # Get metrics with safe defaults (prefer Summary, fallback to sheet counts)
        pipelines = get_count_with_fallback(
            "Pipelines", ["ImpactAnalysis", "PipelineAnalysis", "Pipeline_Analysis", "Pipelines"]
        )
        dataflows = get_count_with_fallback("DataFlows", ["DataFlows", "DataFlowLineage", "DataFlow_Summary"])
        ...
        dependencies = get_count_with_fallback("Total Dependencies", ["ActivityExecutionOrder", "DataLineage", "Pipeline_Pipeline", "Pipeline_DataFlow", "TriggerDetails"]) 
        orphaned = get_count_with_fallback("Orphaned Pipelines", ["OrphanedPipelines", "Orphaned_Pipelines"]) 
    ...
    def render_activity_distribution(self):
        ...
        fig = go.Figure(
            data=[
                go.Pie(
                    labels=activity_df["ActivityType"],
                    values=activity_df["Count"],
                    hole=0.4,
                    marker=dict(
    ...
    def render_resource_summary(self):
        ...
        resource_types = [
            ...
            ("Integration Runtimes", get_summary_metric("Integration Runtimes", 0)),
        ]
    ...
    def render_analysis_info(self):
        ...
        with col2:
            ...
            <div class="info-card">
                <h4>📁 Source File</h4>
                <p title="{source_file}">{truncate_text(filename, 30)}</p>
            </div>
            """,
                unsafe_allow_html=True,
            )
        ...
        with col3:
            ...
            <div class="info-card">
                <h4>🔧 Version</h4>
                <p>{truncate_text(str(version), 30)}</p>
            </div>
            """,
                unsafe_allow_html=True,
            )
    ...
    def render_impact_analysis_tab(self):
        ...
        if selected_pipeline:
            ...
            with col1:
                ...
                # ✅ FIX: Properly render HTML
                st.markdown(f"""
                <div class="info-card" style="border-left: 4px solid {impact_color};">
                    ...
                    <div style="margin: 10px 0;">
                        ...
                        <span class="badge" style="background: {impact_color}; color: white; font-size: 1.1em;">
                            {impact}
                        </span>
                    </div>
                    
                    <div style="margin: 10px 0;">
                        <strong>Blast Radius:</strong> {blast_radius} resources
                    </div>
                    
                    <div style="margin: 10px 0;">
                        <strong>Status:</strong> 
                        {'<span style="color: #FF4444;">⚠️ Orphaned</span>' if is_orphaned == 'Yes' else '<span style="color: #00C851;">✅ Active</span>'}
                    </div>
                </div>
    ...
    def render_orphaned_resources_tab(self):
        ...
        with tabs[0]:
            if orphaned_pipelines.empty:
                ...
                if "Pipeline" in orphaned_pipelines.columns:
                    ...
                    # Export button
                    csv = orphaned_pipelines.to_csv(index=False)
                    st.download_button(
        ...
        with tabs[1]:
            if orphaned_datasets.empty:
                ...
                if "Dataset" in orphaned_datasets.columns:
                    ...
                    csv = orphaned_datasets.to_csv(index=False)
                    st.download_button(
        ...
        with tabs[2]:
            if orphaned_linkedservices.empty:
                ...
                if "LinkedService" in orphaned_linkedservices.columns:
                    ...
                    csv = orphaned_linkedservices.to_csv(index=False)
                    st.download_button(
        ...
        with tabs[3]:
            if orphaned_triggers.empty:
                ...
                csv = orphaned_triggers.to_csv(index=False)
                st.download_button(
        ...
        if total_orphaned == 0:
            ...
            st.markdown(
                f"""
            <div class="info-card" style="border-left: 4px solid #FF8800;">
                <h4 style="color: #FF8800;">⚠️ Action Required</h4>
                <p>Found <strong>{total_orphaned}</strong> orphaned or broken resources.</p>
                
                <h4 style="margin-top: 20px;">Recommended Steps:</h4>
                <ol>
        ...
        if not activity_df.empty:
            ...
            with col2:
                ...
                fig = go.Figure(
                    data=[
                        go.Pie(
                            labels=activity_df["ActivityType"].head(8),
                            values=activity_df["Count"].head(8),
                            hole=0.3,
