...
class UltimateADFParser:
    ...
    # Add these methods to parse discovered resource types
    ...
    def parse_discovered_resources(self):
        ...
        for resource in resources:
            ...
            if 'integrationruntimes' in low:
                self.parse_integration_runtimes(resource)
            if 'managedvirtualnetworks' in low:
                self.parse_managed_virtual_networks(resource)
            if 'linkedservices' in low:
                self.parse_linked_services(resource)
            if 'triggers' in low:
                self.parse_triggers(resource)
            if 'datasets' in low:
                self.parse_datasets(resource)
            if 'dataflows' in low:
                self.parse_dataflows(resource)
            if 'pipelines' in low:
                self.parse_pipelines(resource)

    def parse_integration_runtimes(self, resource: dict):
        """Parse integrationRuntimes resource (generic enhanced)"""
        try:
            name = self.extract_name(resource.get('name', ''))
            ...
            rec = {
                ...
                'resourceId': self.sanitize_value(props.get('resourceId', '')),
                'authorizationType': self.sanitize_value(props.get('authorizationType', '')),
                'linkedInfo': self.sanitize_value(props.get('linkedInfo', '')),
                'type': self.sanitize_value(props.get('type', '')),
                'description': self.sanitize_value(props.get('description', '')),
                'typeProperties': self.sanitize_value(props.get('typeProperties', '')),
                'name': self.sanitize_value(props.get('name', '')),
                'apiVersion': self.sanitize_value(resource.get('apiVersion', '')),
                'properties': self.sanitize_value(props),
                'dependsOn': self.sanitize_value(resource.get('dependsOn', [])),
            }

            self.results.setdefault('integration_runtimes_enhanced', []).append(rec)

        except Exception as e:
            self.log_error(resource, f"integrationRuntimes: {e}")

    def parse_managed_virtual_networks(self, resource: dict):
        ...
        try:
            ...
            rec = {
                ...
                'Name': self.sanitize_value(name),
                'name': self.sanitize_value(props.get('name', '')),
                'type': self.sanitize_value(resource.get('type', '')),
                'apiVersion': self.sanitize_value(resource.get('apiVersion', '')),
                'properties': self.sanitize_value(props),
                'dependsOn': self.sanitize_value(resource.get('dependsOn', [])),
            }

            self.results.setdefault('managed_virtual_networks', []).append(rec)

        except Exception as e:
            self.log_error(resource, f"managedVirtualNetworks: {e}")

    def parse_linked_services(self, resource: dict):
        """Parse linkedServices resource (additional fields)"""
        try:
            name = self.extract_name(resource.get('name', ''))
            props = resource.get('properties', {}) or {}
            ...
            rec = {
                'Type': 'linkedServices',
                'Name': self.sanitize_value(name),
                'baseUrl': self.sanitize_value(self.get_nested(props, 'baseUrl')),
                'annotations': self.sanitize_value(props.get('annotations', [])),
                'type': self.sanitize_value(props.get('type', '')),
                'typeProperties': self.sanitize_value(props.get('typeProperties', {})),
                'name': self.sanitize_value(props.get('name', '')),
                'apiVersion': self.sanitize_value(resource.get('apiVersion', '')),
                'properties': self.sanitize_value(props),
                'dependsOn': self.sanitize_value(resource.get('dependsOn', [])),
                'referenceName': self.sanitize_value(self.get_nested(props, 'referenceName')),
                'store': self.sanitize_value(self.get_nested(props, 'store')),
            }

            self.results.setdefault('linked_services_enhanced', []).append(rec)

        except Exception as e:
            self.log_error(resource, f"linkedServices: {e}")

    def parse_triggers(self, resource: dict):
        """Parse triggers resource (enhanced)"""
        try:
            name = self.extract_name(resource.get('name', ''))
            props = resource.get('properties', {}) or {}
            ...
            rec = {
                'Type': 'triggers',
                'Name': self.sanitize_value(name),
                'referenceName': self.sanitize_value(self.get_nested(props, 'pipelineReference.referenceName')),
                'type': self.sanitize_value(props.get('type', '')),
                'pipelineReference': self.sanitize_value(props.get('pipelineReference', {})),
                'parameters': self.sanitize_value(props.get('parameters', {})),
                'minutes': self.sanitize_value(self.get_nested(props, 'recurrence.schedule.minutes')),
                'hours': self.sanitize_value(self.get_nested(props, 'recurrence.schedule.hours')),
                'frequency': self.sanitize_value(self.get_nested(props, 'recurrence.frequency')),
                'interval': self.sanitize_value(self.get_nested(props, 'recurrence.interval')),
                'startTime': self.sanitize_value(self.get_nested(props, 'recurrence.startTime')),
                'timeZone': self.sanitize_value(self.get_nested(props, 'recurrence.timeZone')),
            }

            self.results.setdefault('triggers_enhanced', []).append(rec)

        except Exception as e:
            self.log_error(resource, f"triggers: {e}")

    def parse_datasets(self, resource: dict):
        """Parse datasets resource (enhanced)"""
        try:
            name = self.extract_name(resource.get('name', ''))
            props = resource.get('properties', {}) or {}
            ...
            rec = {
                'Type': 'datasets',
                'Name': self.sanitize_value(name),
                'referenceName': self.sanitize_value(self.get_nested(props, 'linkedServiceName.referenceName') or self.get_nested(props, 'linkedServiceName')),
                'type': self.sanitize_value(props.get('type', '')),
                'p_schema': self.sanitize_value(self.get_nested(props, 'schema')),
                'p_table': self.sanitize_value(self.get_nested(props, 'typeProperties.tableName')),
                'value': self.sanitize_value(props.get('value', '')),
                'schema': self.sanitize_value(props.get('schema', {})),
                'table': self.sanitize_value(self.get_nested(props, 'typeProperties.tableName')),
                'linkedServiceName': self.sanitize_value(self.get_nested(props, 'linkedServiceName.referenceName') or self.get_nested(props, 'linkedServiceName')),
                'parameters': self.sanitize_value(props.get('parameters', {})),
                'annotations': self.sanitize_value(props.get('annotations', [])),
            }

            self.results.setdefault('datasets_enhanced', []).append(rec)

        except Exception as e:
            self.log_error(resource, f"datasets: {e}")

    def parse_dataflows(self, resource: dict):
        """Parse dataflows resource (generic wrapper to parse_dataflow)"""
        try:
            # delegate to existing parse_dataflow for consistency
            self.parse_dataflow(resource)
        except Exception as e:
            self.log_error(resource, f"dataflows: {e}")

    def parse_pipelines(self, resource: dict):
        """Parse pipelines resource (generic wrapper to parse_pipeline)"""
        try:
            # delegate to existing parse_pipeline for consistency
            self.parse_pipeline(resource)
        except Exception as e:
            self.log_error(resource, f"pipelines: {e}")
