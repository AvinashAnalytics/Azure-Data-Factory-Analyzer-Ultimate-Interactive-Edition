    ...
    def _write_core_data_sheets(self, writer):
        ...
        for sheet_name, data in core_sheets:
            if data:
                ...
                # Write an empty placeholder sheet with no rows but expected columns
                ...
                self._format_sheet(writer, safe_name, freeze_panes=True, auto_filter=True)
                self.logger.warning(f"  ⚠️  {sheet_name}: No data to export - placeholder sheet written")

    def _placeholder_df(self, sheet_name: str) -> pd.DataFrame:
        """Return a sensible empty DataFrame schema for known sheet names.

        This ensures the Excel workbook always contains the expected sheets
        (even when analyzer found no records). The dashboard can then rely on
        sheet existence without needing to synthesize missing sheets.
        """
        schema_map = {
            # Core
            'PipelineAnalysis': ['Pipeline', 'TotalActivities', 'ComplexityScore', 'ImpactLevel'],
            'Pipelines': ['Pipeline', 'Description', 'Folder', 'HasTrigger'],
            'Activities': ['Pipeline', 'Sequence', 'ActivityType', 'Name', 'Depth', 'IntegrationRuntime', 'SQL', 'Tables'],
    ...
    def _write_analysis_sheets(self, writer):
        ...
        for sheet_name, data in analysis_sheets:
            if data:
                ...
                # Write placeholder for analysis sheet when empty
                placeholder = self._placeholder_df(sheet_name)
                safe_name = self._get_unique_sheet_name(sheet_name)
                placeholder.to_excel(writer, sheet_name=safe_name, index=False)
                self._format_sheet(writer, safe_name, freeze_panes=True, auto_filter=True)
    ...
    def _write_usage_statistics_sheets(self, writer):
        ...
        for sheet_name, data in usage_sheets:
            if data:
                ...
                # Write empty placeholder for usage sheet
                placeholder = self._placeholder_df(sheet_name)
                safe_name = self._get_unique_sheet_name(sheet_name)
                placeholder.to_excel(writer, sheet_name=safe_name, index=False)
                self._format_sheet(writer, safe_name, freeze_panes=True, auto_filter=True)
